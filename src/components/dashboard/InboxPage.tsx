...
const EmailListItem = ({
  email, isSelected, onSelect, onMarkRead,
}: {
  email: Email; isSelected: boolean; onSelect: (id: string) => void; onMarkRead: (id: string) => void;
}) => (
  <div
    key={email.id}
    onClick={() => { onSelect(email.id); if (!email.is_read) onMarkRead(email.id); }}
    className={`email-row ${!email.is_read ? "unread" : ""} ${isSelected ? "bg-muted/50" : ""}`}
  >
    <div className="flex-1 min-w-0">
      <div className="flex items-center justify-between">
        <span className={`text-sm font-medium truncate ${!email.is_read ? "text-foreground" : "text-muted-foreground"}`}>
          {email.from_email || "I panjohur"}
        </span>
        <span className="text-xs text-muted-foreground shrink-0 ml-2">{formatTime(email.created_at)}</span>
      </div>
      <div className="text-sm text-foreground truncate mt-0.5">{email.subject || "(Pa subjekt)"}</div>
      ...
      <div className="flex items-center gap-2 mt-1.5">
        {email.category && email.category !== "uncategorized" && (
          <span className={`category-badge ${getCategoryStyle(email.category)}`}>{email.category}</span>
        )}
        {email.is_starred && <Star className="h-3 w-3 text-category-documents fill-category-documents" />}
      </div>
    </div>
  </div>
);
...
const EmailDetail = ({
  email, onToggleStar, onDelete, onBack,
}: {
  email: Email; onToggleStar: () => void; onDelete: () => void; onBack?: () => void;
}) => (
  <>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="font-heading text-lg md:text-xl font-semibold truncate">{email.subject || "(Pa subjekt)"}</h2>
            <p className="text-sm text-muted-foreground mt-1">{email.from_email} · {formatTime(email.created_at)}</p>
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <Button variant="ghost" size="icon" onClick={onToggleStar}>
            <Star className={`h-4 w-4 ${email.is_starred ? "fill-category-documents text-category-documents" : ""}`} />
          </Button>
    ...
    <div className="flex-1 overflow-y-auto scrollbar-thin p-4 md:p-6">
      <div className="prose prose-invert prose-sm max-w-none">
        <p className="text-foreground/80 leading-relaxed whitespace-pre-wrap">
          {email.body || email.snippet || "Pa përmbajtje"}
        </p>
      </div>
    </div>
  </>
...
const InboxPage = () => {
  ...
  const fetchEmails = useCallback(async () => {
    ...
    if (error) {
      toast.error("Dështoi ngarkimi i emaileve");
      console.error(error);
    } else {
      setEmails(data || []);
  ...
  const syncGmail = useCallback(async () => {
    ...
    try {
      ...
      const hasGoogle = user?.app_metadata?.providers?.includes("google") || user?.identities?.some((i) => i.provider === "google");
      if (!hasGoogle) { toast.error("Ju lutem kyçuni me Google për të sinkronizuar inbox-in tuaj Gmail"); return; }

      const res = await supabase.functions.invoke("sync-gmail", {
        body: { provider_token: session.provider_token, provider_refresh_token: session.provider_refresh_token },
      });

      if (res.error) {
        toast.error("Sinkronizimi dështoi: " + (res.error.message || "Gabim i panjohur"));
  ...
  const categorizeEmails = useCallback(async () => {
    ...
    try {
      const emailsToCateg = uncategorized.map((e) => ({
        id: e.id, from: e.from_email, subject: e.subject, body: (e.body || e.snippet || "").slice(0, 500),
      }));
      ...
      for (let i = 0; i < emailsToCateg.length; i += batchSize) {
        ...
        if (res.error) { console.error("Gabim kategorizimi:", res.error); continue; }
        ...
        } catch { console.error("Dështoi analiza e rezultatit të kategorizimit"); continue; }

        for (const r of results) {
          await supabase.from("emails").update({ category: r.category }).eq("id", r.id);
        }

        setEmails((prev) =>
          prev.map((e) => { const match = results.find((r) => r.id === e.id); return match ? { ...e, category: match.category } : e; })
        );
      }
  ...
  useEffect(() => {
    const autoSync = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      const hasGoogle = user?.app_metadata?.providers?.includes("google") || user?.identities?.some((i) => i.provider === "google");
      if (hasGoogle) await syncGmail();
    };
    autoSync();
  }, []);

  const toggleStar = async (emailId: string, currentStarred: boolean) => {
    const { error } = await supabase.from("emails").update({ is_starred: !currentStarred }).eq("id", emailId);
    if (!error) setEmails((prev) => prev.map((e) => e.id === emailId ? { ...e, is_starred: !currentStarred } : e));
  };
        ...
        <div className="w-full md:w-96 border-r border-border/50 flex flex-col shrink-0 glass-card rounded-none border-t-0 border-b-0 border-l-0">
          <div className="p-4 border-b border-border/50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{unreadCount} mesazhe të palexuara</p>
              </div>
              <div className="flex items-center gap-1">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" disabled={categorizing} className="text-xs">
                      {categorizing ? <RefreshCw className="h-3.5 w-3.5 mr-1 animate-spin" /> : <Tags className="h-3.5 w-3.5 mr-1" />}
                      Kategorizo
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={categorizeEmails} className="font-medium">
                      <Tags className="h-4 w-4 mr-2" /> Kategorizo të gjitha
                    </DropdownMenuItem>
                    <div className="h-px bg-border my-1" />
                    {CATEGORIES.map((cat) => (
                      <DropdownMenuItem key={cat.name} onClick={() => setActiveFilter(cat.name)}>
                ...
                <Button variant="ghost" size="icon" onClick={syncing ? undefined : syncGmail} disabled={syncing} title="Sinkronizo Gmail">
            ...
            {activeFilter && (
              <div className="mt-2 flex items-center gap-2">
                <Badge variant="secondary" className="text-xs">
                  Duke shfaqur: {activeFilter} ({filteredEmails.length} emaile)
                </Badge>
                <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" onClick={() => setActiveFilter(null)}>
                  <X className="h-3 w-3 mr-1" /> Pastro filtrin
                </Button>
              </div>
            )}
          </div>
          <div className="flex-1 overflow-y-auto scrollbar-thin">
            {loading && emails.length === 0 ? (
              <div className="flex items-center justify-center h-32 text-muted-foreground text-sm">Duke ngarkuar emailet...</div>
                ...
                {activeFilter ? (
                  ...
                  <>
                    ...
                    <Button variant="outline" size="sm" className="mt-3" onClick={syncGmail} disabled={syncing}>
                      <RefreshCw className={`h-3 w-3 mr-1 ${syncing ? "animate-spin" : ""}`} /> Sinkronizo Gmail
                    </Button>
                  </>
                )}
              </div>
              ...
              filteredEmails.map((email) => (
                <EmailListItem key={email.id} email={email} isSelected={selectedId === email.id} onSelect={setSelectedId} onMarkRead={markAsRead} />
              ))
            )}
          </div>
        </div>
      ...
      {(!isMobile || showDetailMobile) && (
        <div className="flex-1 flex flex-col min-w-0">
          {selected ? (
            <EmailDetail
              email={selected}
              onToggleStar={() => toggleStar(selected.id, !!selected.is_starred)}
